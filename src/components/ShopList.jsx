...
export default function ShopList({ deliveryAddress }) {
  ...
  const loadShopsAndFilter = async () => {
    try {
      ...
      if (deliveryAddress && deliveryAddress.coordinates) {
        const filteredShops = filterShopsByLocation(shops, deliveryAddress);
  ...
  const toggleSubscription = async (shopId) => {
    try {
      ...
      if (isSubscribed) {
        newSubscriptions = userSubscriptions.filter(id => id !== shopId);
  ...
  const subscribedShops = nearbyShops.filter(shop => userSubscriptions.includes(shop._id));
  const availableShops = nearbyShops.filter(shop => !userSubscriptions.includes(shop._id));

  if (loading) {
    return <div style={styles.loading}>🔍 Finding shops that deliver to your area...</div>;
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.deliveryInfo}>
        <h3>🚚 Delivery to: {deliveryAddress?.area}, {deliveryAddress?.city}</h3>
        <p>
        ...
        {deliveryAddress?.pincode && (
          <p><strong>Pincode:</strong> {deliveryAddress.pincode}</p>
        )}
      </div>
        ...
        {subscribedShops.length > 0 ? (
          <div style={styles.shopGrid}>
            {subscribedShops.map(shop => (
              <div key={shop._id} style={styles.subscribedShopCard}>
                <div style={styles.shopInfo}>
                  <h4>{shop.name}</h4>
                  <p>📍 {shop.location?.address || 'Address not available'}</p>
                  <p>📞 {shop.phone || 'No phone'}</p>
                  <p style={styles.distance}>
                    🚚 {shop.distance} km away • Delivers within {shop.deliveryRadius || 5} km
                  </p>
                  {shop.openingHours && (
      ...
      {availableShops.length > 0 && (
        <div style={styles.section}>
          <h3>🔍 Shops Available in Your Area ({availableShops.length})</h3>
          <div style={styles.shopGrid}>
      ...
      {nearbyShops.length === 0 && !loading && (
        <div style={styles.noShops}>
          ...
          <div style={styles.expansionInfo}>
            ...
            <p>Currently showing shops within delivery radius of: <strong>{deliveryAddress?.city}</strong></p>
            <p>Total shops in system: <strong>{allShops.length}</strong></p>
          </div>
        </div>
      )}
    </div>
