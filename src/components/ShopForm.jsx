...
export default function ShopForm() {
  ...
  const findUserShop = async () => {
    try {
      ...
      const userShop = res.data.find(s => {
        console.log('Comparing shop ownerId:', s.ownerId, 'with user._id:', user._id);
        return s.ownerId === user._id || 
  ...
  const loadProducts = async (shopId) => {
    try {
      ...
      const shopProducts = res.data.filter(p => 
        p.shopId === shopId || 
  ...
  const handleCreateShop = async () => {
    try {
      ...
      setShop(res.data);
      alert(`Shop "${res.data.name}" created successfully!\nLocation: ${res.data.location?.city || 'Unknown'}\nDelivery Radius: ${res.data.deliveryRadius} km`);
  ...
  const updateStock = async (productId, newStock) => {
    try {
      const res = await api.put(`/products/${productId}`, { stock: newStock });
    ...
    return (
      <div style={styles.container}>
        <div style={styles.createShopCard}>
          ...
          <p>Set up your shop with precise location for delivery services</p>
          ...
          <div style={styles.form}>
            ...
            <div style={styles.radiusContainer}>
              <label style={styles.radiusLabel}>
                Delivery Radius: <strong>{shopData.deliveryRadius} km</strong>
              </label>
              <input
                type="range"
                name="deliveryRadius"
            ...
            <button 
              ...
              style={creatingShop ? styles.disabledButton : styles.button}
            >
              {creatingShop ? '📍 Creating Shop & Setting Location...' : '🏪 Create Shop with Location'}
            </button>
          </div>
        </div>
      </div>
    ...
    <div style={styles.container}>
      <div style={styles.shopInfo}>
        ...
        <div style={styles.locationDetails}>
          <p><strong>📍 Location:</strong> {shop.location?.formattedAddress || shop.location?.address || 'Location not available'}</p>
          <p><strong>🏙️ City:</strong> {shop.location?.city || 'Unknown'}</p>
          <p><strong>📮 Pincode:</strong> {shop.location?.pincode || 'Unknown'}</p>
          <p><strong>🚚 Delivery Radius:</strong> {shop.deliveryRadius} km</p>
      ...
      <div style={styles.section}>
        ...
        <div style={styles.productForm}>
          ...
          <button 
            ...
            style={addingProduct ? styles.disabledButton : styles.addButton}
          >
            ...
            {products.map(p => (
              <div key={p._id} style={styles.productCard}>
                ...
                <div style={styles.productContent}>
                  ...
                  <p style={styles.productPrice}>₹{p.price} per {p.unit}</p>
                  ...
                  <div style={styles.stockControl}>
                    ...
                    <input
                      ...
                      min="0"
                    />
                    <span style={styles.stockUnit}>{p.unit}</span>
                  </div>
